...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        await user.save();
        await transporter.sendMail({ to: email, subject: "Cloudly OTP", text: `Code: ${otp}` });
...
app.post('/api/auth/login', async (req, res) => {
    ...
    if (!user || !user.isVerified || !(await bcrypt.compare(req.body.password, user.password))) return res.status(400).json({ error: "Invalid credentials" });
