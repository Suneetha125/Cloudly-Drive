...
const app = express();
const SECRET = process.env.JWT_SECRET || "CLOUDLY_FINAL_PRO_2026";
const BUCKET_NAME = 'cloudly';

// Initialize Supabase Official SDK
const supabase = createClient(process.env.SUPABASE_URL, process.env.S3_SECRET_KEY);
...
const User = mongoose.model('User', {
    name: String, email: { type: String, unique: true }, password: { type: String },
    vaultPIN: String, storageUsed: { type: Number, default: 0 }, storageLimit: { type: Number, default: 32212254720 }
});
...
app.get('/api/drive/contents', authenticate, async (req, res) => {
    ...
    if (tab === 'shared') {
        const sharedCriteria = { "sharedWith.email": req.user.email };
        return res.json({ folders: await Folder.find(sharedCriteria), files: await File.find(sharedCriteria) });
    }
...
app.post('/api/upload/complete', authenticate, async (req, res) => {
    try {
        ...
        const { error } = await supabase.storage.from(BUCKET_NAME).upload(s3Path, fileBuffer, { contentType: mimeType });
        if (error) throw error;

        const file = new File({ 
            fileName, fileSize: fileBuffer.length, s3Path, mimeType, 
            parentFolder: (folderId === "null" || !folderId) ? null : folderId, 
            owner: req.user.id, isVault 
        });
        await file.save();
        await User.findByIdAndUpdate(req.user.id, { $inc: { storageUsed: fileBuffer.length } });
        fs.unlinkSync(temp); res.json(file);
