...
// 1. Supabase S3 Setup - FIXED HOSTNAME LOGIC
...
const BUCKET_NAME = 'cloudly'; // MATCHES YOUR SUPABASE BUCKET
...
// 3. SCHEMAS
const User = mongoose.model('User', { name: String, email: { type: String, unique: true }, password: String, vaultPIN: String, isVerified: { type: Boolean, default: false }, otp: String, otpExpires: Date, storageUsed: { type: Number, default: 0 } });
...
app.post('/api/auth/forgot-password', async (req, res) => {
    ...
    await user.save();
    await transporter.sendMail({ to: user.email, subject: "Recovery", text: `Reset code: ${otp}` });
    res.json({ msg: "Sent" });
});
