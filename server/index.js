...
const UserSchema = new mongoose.Schema({
    ...
    password: { type: String, required: true },
    userName: String,
    vaultPin: { type: String, default: '0000' }, // For the Vault feature
    storageUsed: { type: Number, default: 0 },
    storageLimit: { type: Number, default: 100 * 1024 * 1024 } // 100MB Default
});
...
// 1. Fetch Drive Contents (Files and Folders)
app.get('/api/drive/contents', authenticate, async (req, res) => {
    ...
    try {
        ...
        if (tab === 'starred') query.isStarred = true;
            ...
            // Find files where user email is in sharedWith array
            const user = await User.findById(req.user.userId);
            query = { "sharedWith.email": user.email };
        } else {
            // My Drive (Default)
            query.isVault = false;
...
app.post('/api/folders', authenticate, async (req, res) => {
    const { name, parentFolder, isVault } = req.body;
    try {
        const newFolder = new Folder({ name, parentFolder, isVault, owner: req.user.userId });
...
app.post('/api/upload/chunk', authenticate, upload.single('chunk'), (req, res) => {
    ...
    const destDir = path.join(__dirname, 'uploads', uploadId);
    if (!fs.existsSync(destDir)) fs.mkdirSync(destDir, { recursive: true });

    fs.renameSync(chunkPath, path.join(destDir, partNumber));
    res.json({ message: "Chunk uploaded" });
});

app.post('/api/upload/complete', authenticate, async (req, res) => {
    const { uploadId, fileName, folderId, isVault, mimeType, fileSize } = req.body;
    const destDir = path.join(__dirname, 'uploads', uploadId);
    const finalPath = path.join(__dirname, 'uploads', `${uploadId}-${fileName}`);

    // Merge chunks
    const files = fs.readdirSync(destDir).sort((a, b) => a - b);
    const writeStream = fs.createWriteStream(finalPath);
    for (const file of files) {
        const data = fs.readFileSync(path.join(destDir, file));
    ...
    const newFile = new File({
        fileName, fileSize, mimeType, path: finalPath,
        owner: req.user.userId, parentFolder: folderId, isVault
    });
    await newFile.save();
    await User.findByIdAndUpdate(req.user.userId, { $inc: { storageUsed: fileSize } });
...
app.patch('/api/drive/star/:type/:id', authenticate, async (req, res) => {
    ...
    const { isStarred } = req.body;
    const Model = type === 'file' ? File : Folder;
    await Model.findByIdAndUpdate(id, { isStarred });
...
app.delete('/api/drive/delete/:type/:id', authenticate, async (req, res) => {
    ...
    if (type === 'file') {
        ...
        if (file) {
            ...
            await User.findByIdAndUpdate(req.user.userId, { $inc: { storageUsed: -file.fileSize } });
            await File.findByIdAndDelete(id);
        }
    } else {
        await Folder.findByIdAndDelete(id); // Simple delete, doesn't handle sub-items for now
    }
...
app.post('/api/vault/unlock', authenticate, async (req, res) => {
    const { pin } = req.body;
    const user = await User.findById(req.user.userId);
    if (user.vaultPin === pin) res.json({ message: "Unlocked" });
...
app.post('/api/files/share', authenticate, async (req, res) => {
    ...
    await File.findByIdAndUpdate(fileId, {
        $push: { sharedWith: { email, role, expiresAt } }
    });
...
app.patch('/api/drive/move', authenticate, async (req, res) => {
    ...
    const Model = type === 'file' ? File : Folder;
    await Model.findByIdAndUpdate(itemId, { parentFolder: targetId });
...
app.get('/api/drive/preview/:id', authenticate, async (req, res) => {
    ...
    // In a real app, you'd serve the file or a signed URL
    res.json({ url: `http://localhost:5000/download/${file._id}` }); 
});

// 11. Delete Account
app.delete('/api/auth/delete-account', authenticate, async (req, res) => {
    await User.findByIdAndDelete(req.user.userId);
    await File.deleteMany({ owner: req.user.userId });
    await Folder.deleteMany({ owner: req.user.userId });
