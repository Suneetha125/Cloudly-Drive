...
app.post('/api/auth/forgot-password', async (req, res) => {
    ...
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    user.otp = otp; user.otpExpires = Date.now() + 600000; await user.save();
    await transporter.sendMail({ from: process.env.BREVO_USER, to: user.email, subject: 'Cloudly OTP', text: `OTP: ${otp}` });
    res.json({ success: true });
});
...
app.post('/api/upload/complete', authenticate, async (req, res) => {
    ...
    const file = new File({ fileName, fileSize: stats.size, s3Path, mimeType, parentFolder: (folderId === "null" || !folderId) ? null : folderId, owner: req.user.id, isVault: isVault === 'true' || isVault === true });
    await file.save();
