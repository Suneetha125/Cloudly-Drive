        ...
        <div style={styles.container}>
            <div style={styles.card}>
                <div style={{textAlign:'center', marginBottom:30}}>
                    ...
                    <h2 style={{marginTop:10}}>{mode === 'login' ? 'Sign In' : mode === 'signup' ? 'Create Account' : 'Verify Email'}</h2>
                </div>
                <form onSubmit={handleSubmit} style={{display:'flex', flexDirection:'column', gap:15}}>
                    ...
                    <button style={styles.btn} type="submit">{mode === 'login' ? 'Login' : 'Continue'}</button>
                    <p onClick={() => setMode(mode === 'login' ? 'signup' : 'login')} style={styles.toggle}>{mode === 'login' ? "New here? Sign up" : "Have an account? Login"}</p>
                </form>
            </div>
        </div>
    );
...
const styles = {
    ...
    card: { background: '#fff', padding: 40, borderRadius: 24, boxShadow: '0 10px 25px rgba(0,0,0,0.05)', width: 400 },
