...
const Auth = () => {
    ...
    e.preventDefault();
    try {
        // Force lowercase email to prevent case-sensitive login errors
        const loginData = { 
        ...
        if (isLogin) {
            ...
            alert("Success! Now please login with your new account.");
            setIsLogin(true);
        }
    } catch (err) {
        // This will show you the EXACT error from the backend in an alert
        ...
        <div style={styles.container}>
            <div style={styles.card}>
                <div style={{textAlign:'center', marginBottom:30}}>
                    ...
                    <h2 style={{marginTop:10, color: '#1e293b'}}>{isLogin ? 'Login to Cloudly' : 'Create Startup Account'}</h2>
                    <p style={{color:'#64748b', fontSize:14}}>{isLogin ? 'Enter your credentials' : 'Join our secure storage platform'}</p>
                </div>
...
const styles = {
    ...
    btn: { padding: 14, background: '#3b82f6', color: '#fff', border: 'none', borderRadius: 12, cursor: 'pointer', fontWeight: 'bold', fontSize: 16, marginTop: 10, transition: 'all 0.2s' },
