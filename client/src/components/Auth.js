...
const Auth = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            alert(err.response?.data?.error || "Auth failed. Try a different email."); 
        }
    };
        ...
        <div style={styles.container}>
            <div style={styles.card}>
                ...
                <form onSubmit={handleSubmit} style={{display:'flex', flexDirection:'column', gap:15}}>
                    {!isLogin && <div style={styles.inputGroup}><User size={18} color="#94a3b8"/><input style={styles.input} placeholder="Name" onChange={e=>setForm({...form, name: e.target.value})} required /></div>}
                    <div style={styles.inputGroup}><Mail size={18} color="#94a3b8"/><input style={styles.input} type="email" placeholder="Email" onChange={e=>setForm({...form, email: e.target.value})} required /></div>
                    <div style={styles.inputGroup}>
                        <Lock size={18} color="#94a3b8"/>
                        <input style={styles.input} type={showPassword ? "text" : "password"} placeholder="Password" onChange={e=>setForm({...form, password: e.target.value})} required />
                    ...
                    <p onClick={() => setIsLogin(!isLogin)} style={{cursor:'pointer', color:'#3b82f6', textAlign:'center', fontSize:14}}>{isLogin ? "Need an account? Sign up" : "Have an account? Login"}</p>
                </form>
            </div>
        </div>
    );
