        ...
        <div style={styles.container}>
            <div style={styles.card}>
                <div style={{textAlign:'center', marginBottom:30}}>
                    ...
                    <h2>{showOtp ? 'Verify Email' : isLogin ? 'Login' : 'Sign Up'}</h2>
                </div>
                <form onSubmit={handleSubmit} style={{display:'flex', flexDirection:'column', gap:15}}>
                    {showOtp ? (
                        ...
                        <>
                            {!isLogin && <div style={styles.inputGroup}><User size={18}/><input style={styles.inputNo} placeholder="Name" onChange={e=>setForm({...form, name: e.target.value})} required /></div>}
                            <div style={styles.inputGroup}><Mail size={18}/><input style={styles.inputNo} type="email" placeholder="Email" onChange={e=>setForm({...form, email: e.target.value})} required /></div>
                    ...
                    <button style={styles.btn} type="submit">{showOtp ? 'Verify' : isLogin ? 'Sign In' : 'Get Started'}</button>
                    <p onClick={() => {setIsLogin(!isLogin); setShowOtp(false)}} style={{cursor:'pointer', color:'#3b82f6', textAlign:'center', fontSize:14}}>{isLogin ? "Need an account? Sign up" : "Already have an account? Login"}</p>
                </form>
            </div>
        </div>
    );
