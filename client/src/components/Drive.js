                        ...
                        <div key={f._id} style={{...styles.card, background:theme.card, borderColor:theme.border}} onDoubleClick={()=>{if(currentFolder?._id !== f._id){setPath([...path, f]); setCurrentFolder(f)}}}>
                            ...
                            {activeMenu === f._id && (
                                <div style={{...styles.drop, background:theme.card, border:`1px solid ${theme.border}`}}>
                                    ...
                                    <div onClick={()=>handleAction(f._id, 'folder', 'star', !f.isStarred)}>Star</div>
                                    <div onClick={()=>{const t=prompt("Target ID (root, vault, or ID)"); if(t) handleAction(f._id, 'folder', 'move', t)}}>Move</div>
                                    <div onClick={()=>handleAction(f._id, 'folder', 'move', 'trash')} style={{color:'orange'}}>Trash</div>
                                    <div onClick={async ()=>{if(window.confirm("Permanent?")) await axios.delete(`${API}/drive/delete/folder/${f._id}`, authConfig()); fetchData();}} style={{color:'red'}}>Delete</div>
                                </div>
                            )}
                        </div>
                    ))}
                        ...
                        <div key={f._id} style={{...styles.card, background:theme.card, borderColor:theme.border}}>
                            ...
                            {activeMenu === f._id && (
                                <div style={{...styles.drop, background:theme.card, border:`1px solid ${theme.border}`}}>
                                    ...
                                    <div onClick={async ()=>{if(window.confirm("Permanent?")) await axios.delete(`${API}/drive/delete/file/${f._id}`, authConfig()); fetchData();}} style={{color:'red'}}>Delete</div>
                                </div>
                            )}
                        </div>
                    ))}
