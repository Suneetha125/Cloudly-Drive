...
import { 
  FolderPlus, Upload, Trash2, FileText, Search, LogOut, Folder, ChevronRight, Sun, Moon, 
  MoreVertical, Share2, Users, Star, Shield, LayoutGrid, Eye, Move, Download 
} from 'lucide-react';
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: theme.bg, color: theme.text, display: 'flex' }} onClick={() => setActiveMenu(null)}>
            <aside style={{ width: 260, borderRight: `1px solid ${theme.border}`, padding: 20, background: theme.card, display:'flex', flexDirection:'column', gap:10 }}>
                ...
                <div style={{display:'flex', gap:12, padding:12, cursor:'pointer', borderRadius:8, background: activeTab === 'shared' ? theme.accent : 'transparent', color: activeTab === 'shared' ? '#fff' : theme.text}} onClick={() => {setActiveTab('shared'); setCurrentFolder(null); setPathHistory([]);}}><Users size={20}/> Shared</div>
                <div style={{display:'flex', gap:12, padding:12, cursor:'pointer', borderRadius:8, background: activeTab === 'starred' ? theme.accent : 'transparent', color: activeTab === 'starred' ? '#fff' : theme.text}} onClick={() => {setActiveTab('starred'); setCurrentFolder(null); setPathHistory([]);}}><Star size={20}/> Starred</div>
                <div style={{display:'flex', gap:12, padding:12, cursor:'pointer', borderRadius:8, background: activeTab === 'vault' ? theme.accent : 'transparent', color: activeTab === 'vault' ? '#fff' : theme.text}} onClick={async ()=>{
                    const pin = prompt("Vault PIN:");
                    if(pin) {
                        try {
                            await axios.post(`${API}/vault/unlock`, {pin}, authConfig());
                            setIsVaultUnlocked(true); setActiveTab('vault'); setCurrentFolder(null); setPathHistory([]);
                        ...
                        {pathHistory.map((h, i) => (
                            <span key={i} style={{display:'flex', alignItems:'center', gap:5}}>
                                ...
                                <span style={{cursor:'pointer'}} onClick={()=>{
                                    const newHistory = pathHistory.slice(0, i);
