...
const Drive = () => {
    ...
    const handleMove = async (targetId, toVault = false) => {
        await axios.patch(`${API}/drive/move`, { itemId: moveModal.id, type: moveModal.type, targetId, toVault }, authConfig());
        setMoveModal(null); fetchData();
    };

    const handleBiometric = () => {
        alert("Authenticating Fingerprint...");
        ...
        <div style={{ minHeight: '100vh', backgroundColor: theme.bg, color: theme.text, display: 'flex' }} onClick={() => {setActiveMenu(null); setProfileOpen(false)}}>
            ...
            {moveModal && (
                <div style={styles.overlay} onClick={()=>setMoveModal(null)}>
                    <div style={{...styles.modalSmall, background:theme.card}} onClick={e=>e.stopPropagation()}>
                        <h3>Move to...</h3>
                        <div onClick={()=>handleMove('root')} style={styles.moveOption}>My Drive (Root)</div>
                        <div onClick={()=>handleMove('vault_root', true)} style={styles.moveOption}>Vault (Root)</div>
                        {foldersList.filter(f => f._id !== moveModal.id).map(folder => (
                            <div key={folder._id} onClick={()=>handleMove(folder._id)} style={styles.moveOption}>{folder.name}</div>
                        ))}
                    </div>
                </div>
            )}
