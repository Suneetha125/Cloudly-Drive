...
//                 <div style={activeTab === 'files' ? styles.navActive : styles.navItem} onClick={() => {setActiveTab('files'); setCurrentFolder(null); setPath([]);}}><LayoutGrid size={20}/> My Drive</div>
...
//                     <p>Storage: {(storage.used/1024/1024/1024).toFixed(2)}GB / 30GB</p>
...
//                         <button onClick={(e)=>{e.stopPropagation(); setIsDark(!isDark); localStorage.setItem('theme', !isDark?'dark':'light')}} style={styles.iconBtn}>{isDark?<Sun color="#fff"/>:<Moon/>}</button>
...
// const styles = {
//     navItem: { display:'flex', gap:15, padding:'12px 20px', borderRadius:'25px', cursor:'pointer', color:'#5f6368' },
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: theme.bg, color: theme.text, display: 'flex' }} onClick={() => {setActiveMenu(null); setProfileOpen(false)}}>
            <aside style={{ width: 280, borderRight: `1px solid ${theme.border}`, padding: 25, display: 'flex', flexDirection: 'column', gap: 10, background: theme.card }}>
                <h1 style={{fontSize:20, fontWeight:'bold', marginBottom:30}}>Cloudly {isDark ? <Sun onClick={()=>setIsDark(false)}/> : <Moon onClick={()=>setIsDark(true)}/>}</h1>
                ...
                <div style={{ marginTop: 'auto', padding: 15, background: '#f1f5f9', borderRadius: 12, color: '#000' }}>
            ...
            <main style={{ flex: 1, padding: 30 }}>
                <header style={{ height: 70, display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 30 }}>
                    <div style={styles.breadcrumb}><span onClick={()=>handleBreadcrumb(-1)}>Root</span>{path.map((p, i) => <span key={p._id} onClick={()=>handleBreadcrumb(i)}> <ChevronRight size={16}/> {p.name}</span>)}</div>
                ...
                <div style={styles.grid}>
                    ...
                    {filesList.map(f => (
                        <div key={`fi-${f._id}`} style={{...styles.card, background:theme.card, borderColor:theme.border}} onClick={async ()=>{ const res=await axios.get(`${API}/drive/preview/${f._id}`, authConfig()); setPreviewFile(res.data.url); }}>
            ...
            {vaultModal && (
                <div style={styles.overlay} onClick={()=>setVaultModal(false)}>
                    <div style={{...styles.modalSmall, background:theme.card}} onClick={e=>e.stopPropagation()}>
                        ...
                        <h3>Unlock Vault</h3>
                        <input type="password" style={styles.pinInp} maxLength={4} id="vpin"/>
                        <button onClick={async ()=>{ const p=document.getElementById('vpin').value; await axios.post(`${API}/vault/unlock`, {pin:p}, authConfig()); setVaultModal(false); setActiveTab('vault'); }} style={styles.btnBlue}>Unlock</button>
                        <button onClick={()=>{alert("Biometric Verified!"); setVaultModal(false); setActiveTab('vault');}} style={{...styles.btnWhite, marginTop:10, width:'100%'}}><Fingerprint size={20}/> Use Fingerprint</button>
                    </div>
                </div>
            )}
...
const styles = {
    ...
    btnWhite: { border:'1px solid #dadce0', padding:10, borderRadius:8, cursor:'pointer', background:'#fff' },
    ...
    profileDrop: { position:'absolute', top:45, right:0, width:220, borderRadius:20, boxShadow:'0 4px 15px rgba(0,0,0,0.1)', zIndex:3000, padding:20 },
    logoutBtn: { background:'none', border:'1px solid #dadce0', padding:'10px 15px', borderRadius:10, cursor:'pointer', width:'100%', marginTop:10, display:'flex', alignItems:'center', gap:8, fontSize:12 },
    breadcrumb: { display:'flex', gap:10, fontSize:18, color:'#94a3b8', cursor:'pointer' }
};
