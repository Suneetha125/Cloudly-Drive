...
const Drive = () => {
    ...
    const handleDeleteAccount = async () => {
        if (window.confirm("Delete account and ALL files permanently?")) {
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: theme.bg, color: theme.text, display: 'flex' }} onClick={() => {setActiveMenu(null); setProfileOpen(false)}}>
            ...
            <aside style={{ width: '280px', borderRight: `1px solid ${theme.border}`, padding: '30px 15px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                ...
                <div style={activeTab === 'vault' ? styles.navActive : styles.navItem} onClick={()=>{const p = prompt("PIN:"); p && axios.post(`${API}/vault/unlock`, {pin:p}, authConfig()).then(()=>setActiveTab('vault'))}}><Shield size={20} color="#ef4444"/> Vault</div>
            ...
            <main style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                <header style={{ height: '80px', padding: '0 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${theme.border}` }}>
                    ...
                    <div style={{ display: 'flex', gap: '20px', alignItems: 'center', position:'relative' }}>
                        <button onClick={(e)=>{e.stopPropagation(); setIsDark(!isDark)}} style={styles.iconBtn}>{isDark?<Sun/>:<Moon/>}</button>
                ...
                <div style={{ padding: '40px', flex: 1, overflowY: 'auto' }}>
                    <div style={styles.breadcrumb}>
                        ...
                        {path.map((p, i) => <span key={p._id} style={{cursor:'pointer'}}> <ChevronRight size={16} style={{display:'inline'}}/> {p.name}</span>)}
                            ...
                            <div key={`file-${f._id}`} style={{...styles.card, backgroundColor: theme.card, borderColor: theme.border}}>
                                ...
                                {activeMenu === f._id && (
                                    <div style={{...styles.drop, backgroundColor: theme.card, border:`1px solid ${theme.border}`}}>
                                        ...
                                        <div onClick={(e)=>{e.stopPropagation(); axios.delete(`${API}/files/${f._id}`, authConfig()).then(fetchData)}}><Trash2 size={14}/> Delete</div>
                                    </div>
                                )}
                            </div>
                        ))}
